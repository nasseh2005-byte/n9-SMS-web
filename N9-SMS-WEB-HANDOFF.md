...
> حالة التطبيق: يعمل محلياً، وله نشر Sites خاص، ومجهز لنشر Vercel محلي البيانات؛ الاختبارات الآلية الحالية ناجحة 23/23.
...
**N9 SMS Web** هو تطبيق ويب عربي بالأساس، مصمم لإدارة أرشيفات رسائل SMS لكل شركة على حدة، ثم البحث داخل الرسائل ومطابقة أرقام مرجعية قادمة من Excel، واختيار الرسالة الصحيحة يدوياً أو تلقائياً، وعرضها في واجهة تحاكي تطبيقات الرسائل على الهاتف، ثم تصدير صورة دليل عالية الدقة أو ملف ZIP/CSV.
...
9. نسخ النص أو تصدير صورة/ZIP/CSV.
...
- `StatusBar`: وقت الرسالة أو وقت حي يتحدث كل 30 ثانية أو وقت مخصص.
...
- `EvidencePhone`: يعرض دليل الرسالة والتواريخ والنوع والمرسل.
...
- `selectionMode`, `candidateQuery`, `candidateSort`
...
- الرسالة المحددة فقط.
- اسم الملف يتضمن المرسل وID.
- الصورة بدقة 2x.

### ZIP

- يحتوي صورة لكل دليل مختار من مجموعات المطابقة.
- أسماء الملفات تنظف الرموز غير الصالحة لنظام الملفات.
- تحديث progress كل 10 صور تقريباً.
- ضغط DEFLATE مستوى 6.
...
- الثيم الافتراضي داخل التطبيق داكن.
- يمكن التبديل إلى فاتح.
...
- فلترة بالتاريخ والمرسل والاتجاه.
...
- كل المرشحين موجودون، manual/auto selection.
