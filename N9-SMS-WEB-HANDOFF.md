...
> آخر تحديث توثيقي: 10 سبتمبر 2026 — توقيت الرياض
> حالة التطبيق: يعمل محلياً، وله نشر Sites خاص، ونسخة Vercel إنتاجية مفعلة بتخزين مشترك عبر Neon وPrivate Vercel Blob؛ خط تحقق تحديث صلاحيات المحادثات واستيراد XML هو 63/63. الأعداد في الأقسام التاريخية تمثل وقت كتابتها.
> رابط الإنتاج الأساسي: `https://sms-kappa-beige.vercel.app`
> مستودع المصدر: `https://github.com/nasseh2005-byte/n9-SMS-web` — الفرع `main`.
> آخر خط أساس وظيفي قبل تحديث هذا المرجع: `c3cd415`، ويجب دائماً الرجوع إلى `git log -1` لمعرفة الإصدار الأحدث.
> هذا الملف هو المرجع القانوني الأول للمشروع ونقطة البداية لأي مطور أو وكيل Codex يستلمه لاحقاً.

---

## 1. لماذا يوجد هذا الملف؟

هذا مستند تسليم شامل لمشروع **N9 SMS Web**. الهدف منه أن يستطيع Codex آخر فهم المنتج وبنيته وقراراته الحساسة خلال دقائق، ثم يكمل التطوير من دون إعادة اكتشاف المتطلبات أو كسر سلامة البيانات. لا تنشئ مرجعاً موازياً يكرر هذا الملف؛ حدّث هذا الملف نفسه عند تغير قرار دائم أو تدفق أساسي أو بنية تخزين أو نشر.
...
|---|---|
| المنتج | مساحة عمل لإدارة أرشيفات SMS ومطابقة الأرقام وتصدير الأدلة |
| اللغة | العربية أولاً، مع واجهة إنجليزية قبل تسجيل الدخول |
| الثيم الافتراضي | فاتح لؤلؤي؛ الداكن كحلي/أزرق اختياري ومحفوظ |
| تسجيل الدخول | خادمي في Vercel، والجلسات مشتركة عبر الأجهزة |
| البيانات المنظمة | Neon Postgres |
| أرشيفات الشركات | Private Vercel Blob، كائن JSON مستقل لكل شركة |
| الاستيراد | XML للرسائل، وExcel/CSV لأرقام المطابقة |
| أنظمة الهاتف | Google Android وHuawei EMUI وApple iPhone |
| التصدير | PNG وPDF وZIP وCSV، وتصدير محادثة كاملة إلى ZIP يحوي PDF لكل رسالة وExcel مرجعي |
| الاختبارات | 47 ناجحاً، 0 فشل |
| الإنتاج | `https://sms-kappa-beige.vercel.app` |
...
- تصدير المحادثة الكاملة: PDF مستقل لكل رسالة، وExcel عربي مرجعي يحتوي الروابط في عمود مستقل، ثم ZIP واحد مع نسبة تقدم حية.
- حفظ محلي في وضع التطوير، وحفظ سحابي D1/R2 في Sites أو Neon/private Blob في Vercel.

الهوية البصرية مستوحاة بقوة من **Google Messages** في مساحة الرسائل، بينما يحمل Chrome الموقع هوية N9 الخاصة: لؤلؤ وعاج مع شامبانيا ذهبية في الفاتح، وكحلي عميق مع أزرق كهربائي في الداكن. هذه الهوية الخارجية/الداخلية لا تغيّر شاشات الهاتف أو لقطات الأدلة المصدرية.
...
- ينشئ الرسائل اليدوية ويربطها بمحادثة قائمة أو جديدة.
...
- يصدر الأدلة.
- لا يدير المستخدمين، ولا ينشئ الشركات، ولا يرى أو يستخدم منشئ الرسالة.
...
- ثيم ترحيب فاتح وداكن محفوظ على الجهاز، والفاتح هو الافتراضي للمستخدم الجديد.
- النص عملي ومباشر ويشرح مسار XML ← المطابقة ← المراجعة والتصدير بدل عبارات تسويقية عامة.
- بطاقة الدخول تبقى واضحة وبارزة، وتتحول الصفحة إلى عمود واحد قبل أن تنضغط المقدمة أو حقول الدخول في العروض المتوسطة.
- اتجاه الواجهة الخارجية هو «أرشيف رقمي آمن»: كحلي عميق مع أزرق كهربائي وتركوازي محدود، وبطاقات مؤسسية زجاجية. الصورة `public/og.png` هي الأصل البصري الداكن، لكن الثيم الفاتح الذهبي هو الافتراضي للترحيب مع بقاء الداكن اختياراً محفوظاً.
- الواجهة الخارجية مسرح صورة ثابتة كامل الشاشة: `public/og.png` للداكن، و`public/n9-hero-light.png` للفاتح اللؤلؤي ذي النيون الذهبي. `public/n9-logo-pearl-512.png` شعار الوضع الفاتح، و`public/n9-logo-night-512.png` الشعار الليلي الكحلي ذي النيون الأزرق، و`public/favicon-256.png` أيقونة المتصفح. لا توجد فوق المجلدات الثلاث أي مناطق Hover أو نقر أو بطاقات شرح أو أزرار ميزات على الجوال؛ أزيلت عمداً لأن محاذاة مناطق التفاعل مع صورة Raster لا تبقى منطقية عبر أحجام العرض. تسجيل الدخول يظهر بعد ضغط زر الدخول ثم حركة فتح القفل وانزلاق اللوحة من الجانب (ومن الأسفل على الجوال).
- اسم المستخدم لا يملأ مسبقاً باسم المشرف، ويبدأ فارغاً. داخل لوحة الدخول يوجد زر نصي واضح للعودة إلى الواجهة الرئيسية إضافة إلى زر الإغلاق والخلفية القابلة للإغلاق.

### 7.2 بعد تسجيل الدخول

تتكون الشاشة الرئيسية من أربع مناطق وظيفية:

1. **شريط تنقل جانبي**: الرسائل، المطابقة، الملفات، الشركات، ومنشئ الرسالة وإدارة المستخدمين للمشرف فقط.
...
- الميزة محصورة بالمشرف فقط؛ جميع مداخلها مخفية عن المستخدم العادي، كما يُرفض تغيير الرسائل ذات `sourceKind: "manual"` من الخادم إن لم يكن صاحب الجلسة مشرفاً.
...
├─ public/
│  ├─ og.png                       # المسرح الخارجي الداكن؛ لا تستبدله بلا سبب
│  ├─ n9-hero-light.png            # المسرح الخارجي اللؤلؤي الفاتح
│  ├─ n9-logo-pearl-512.png        # شعار N9 للثيم الفاتح
│  ├─ n9-logo-night-512.png        # شعار N9 للثيم الليلي
│  └─ favicon-256.png              # أيقونة المتصفح الفعلية
├─ scripts/
...
│  ├─ archivePermissions.js        # حماية الرسائل المنشأة يدوياً حسب الدور
│  ├─ conversationExportUtils.js   # صفوف Excel واستخراج روابط المحادثة
│  ├─ dataUtils.js                 # التطبيع، Excel، المطابقة، التواريخ، المنشئ
│  ├─ pdfUtils.js                  # أسماء الأدلة وتحويل JPEG إلى PDF صالح
│  ├─ zipUtils.js                  # بناء ZIP تدريجي للمحادثات الكبيرة
...
├─ tests/
│  ├─ composer-permissions.test.mjs # حصر المنشئ بالمشرف عبر كل طبقات الحفظ
│  ├─ conversation-export-utils.test.mjs # مرجع المحادثة والروابط والتواريخ
│  ├─ data-utils.test.mjs          # اختبارات البيانات والمطابقة والمنشئ
│  ├─ iphone-export.test.mjs       # حماية شكل تصدير iPhone المختصر
│  ├─ pdf-utils.test.mjs           # اختبارات أسماء الملفات وبنية PDF
│  ├─ sites-worker.test.mjs        # اختبارات auth/static fallback/package
│  ├─ theme-brand.test.mjs         # الشعار والفافيكون وعزل الثيم عن الأدلة
│  ├─ vercel-api.test.mjs          # تشفير ومسارات ومخطط API السحابي
│  ├─ vercel-config.test.mjs       # إعداد build/API/Blob لـVercel
│  └─ zip-utils.test.mjs           # سلامة ZIP وأسماء الملفات العربية
├─ worker/
...
- `StatusBar`: وقت الرسالة أو وقت حي يتحدث كل 30 ثانية أو وقت مخصص. Android وHuawei يستخدمان أشرطة اتصال متدرجة ورمز Wi-Fi مرسوماً بنسبة صحيحة وبطارية ذات إطار وامتلاء، بدلاً من أيقونات عامة تبدو مصطنعة؛ مسار iPhone المحمي لم يتغير.
- ضبط شريط Android/Huawei الأخير: ارتفاع 42px، ساعة 13px، هوامش أفقية 22px، ومحاذاة فتحة الكاميرا والرموز على منتصف الشريط. أزيلت كتابة 4G الصغيرة بجانب Wi-Fi لتخفيف ازدحام المؤشرات. هذه عناصر عرض وليست بيانات جهاز مستخرجة من XML.
...
- `EvidencePhone`: يوزع التصدير بحسب النظام؛ Android/Huawei يعرضان شاشة التفاصيل الواقعية، بينما iPhone يحيل إلى `IphoneEvidencePhone` الذي يصدر لقطة محادثة مختصرة فقط.
- `IphoneEvidencePhone`: يعرض خطاً علوياً رفيعاً، المرسل، تاريخ الرسالة الأصلي، فقاعة واحدة، سهم الرجوع، وصورة اتصال فقط عندما يكون المرسل رقماً.
...
الصلاحية إدارية فقط: الواجهة لا تعرض زر المنشئ أو حواره للمستخدم العادي، و`handleCreateManualMessage` يعيد الرفض قبل البناء، ثم تتحقق طبقات Local وSites وVercel من أن غير المشرف لم يضف أو يعدل أو يحذف أي رسالة يدوية عند حفظ الأرشيف. يستطيع المستخدم العادي إبقاء الرسائل التي أنشأها المشرف ضمن الأرشيف واستيراد XML جديد من دون حذفها.
...
نسخة iPhone في **عرض المحادثة** تتبع تشريحاً بصرياً أقرب إلى iMessage: إطار معدني وأزرار جانبية، Dynamic Island بعدسة، شريط حالة iOS، رأس محادثة زجاجي، فقاعات زرقاء/رمادية بذيول، بحث داخلي، وصف كتابة بكاميرا وميكروفون.

أما **دليل الرسالة الواحدة وتصديره** فلا يستخدم شاشة «التفاصيل» ولا إطار الجهاز. مرجعه البصري هو `C:\Users\win\Downloads\1734198-1-3.pdf`، ويظهر كلقطة محادثة أفقية بنسبة المرجع: خط علوي، اسم/رقم المرسل، تاريخ XML الأصلي، فقاعة واحدة بلسان iMessage، سهم رجوع أسفل اليسار بمحاذاة نهاية الفقاعة، وصورة اتصال فقط للمرسل الرقمي. لا تظهر الحالة أو النوع أو الأولوية أو شريط حالة الجهاز أو Dynamic Island أو بيانات N9 داخل اللقطة.
...
- شاشة التفاصيل في Android وHuawei تتبع مرجع الهاتف الحقيقي: لا بطاقة هوية مرسل أعلى النص، ولا شارة `SMS`، ولا بطاقة مصدر/N9 في الأسفل. تظهر مساحة الرسالة ثم حالتا الاستلام والإرسال ثم النوع والأولوية بتسلسل بسيط.
- تخطيط Android/Huawei يلتقط مساحة `393×852px`، بينما iPhone يلتقط مساحة أفقية `562.5×441.75px` عند كثافة 2× لتنتج صورة تقارب `1125×884px` مثل مرجع PDF.
- نص iPhone يتدرج حجمه آلياً بحسب طول الرسالة كي تبقى الرسالة كاملة داخل الفقاعة. الأرقام الطويلة لا تنكسر، والروابط تبقى LTR ومعزولة بصرياً داخل الفقاعة.
...
### تصدير المحادثة كاملة

- زر «المحادثة» في شريط المعاينة يعمل على كل رسائل المحادثة المفتوحة، ولا يعتمد على نتائج المطابقة أو اعتماد رسالة يدوياً.
- كل رسالة تتحول إلى PDF مستقل من صفحة واحدة وباسم فريد مرتب يبدأ بتسلسل صفري مثل `001-...pdf`، وتوضع الملفات داخل مجلد `PDF` في الأرشيف.
- يضاف ملف Excel عربي باسم مرجع المحادثة، ويحتوي التسلسل، المحادثة، المرسل، الاتجاه، التواريخ الأصلية المحفوظة، النص الكامل، اسم PDF، المصدر، ومعرف الرسالة.
- الروابط المستخرجة من نص كل SMS تحفظ في عمود مستقل اسمه `روابط موجودة في الرسالة`؛ عدة روابط تحفظ داخل الخلية نفسها على أسطر منفصلة.
- التصدير يعمل كمهمة خلفية غير حاجبة؛ يستطيع المستخدم التنقل واستعمال مساحة العمل أثناء التنفيذ.
- إشعار تقدم عائم وقابل للتصغير يعرض النسبة والمرحلة وعدد الرسائل المنجزة منذ أول PDF حتى إغلاق ZIP وبدء التنزيل، ولا يغطي الواجهة بطبقة Modal.
- إعدادات الهاتف والثيم والساعة تُثبّت عند بدء المهمة كي تبقى كل ملفات PDF في الحزمة متسقة حتى لو غيّر المستخدم الواجهة أثناء التصدير.
- التنفيذ يستخدم `StoredZipBuilder` في `src/zipUtils.js` لإضافة ملفات PDF وExcel تدريجياً دون الاحتفاظ بنسخ غير مضغوطة إضافية مثل تجميع JSZip التقليدي، وهو المسار المخصص للمحادثات الكبيرة.
...
- في إنتاج Vercel يجب تمريره من متغير البيئة السري `BOOTSTRAP_ADMIN_PASSWORD`، ولا يوجد fallback متصفح أو قيمة مضمنة في الحزمة.
- Worker التاريخي الخاص بـSites فقط قد يحتفظ بمسار bootstrap أقدم؛ لا تنسخ هذا السلوك إلى Vercel، وعالجه كدين أمني قبل توسيع مشاركة نسخة Sites.
...
- الواجهة الداخلية الفاتحة تستخدم أسطحاً لؤلؤية عاجية وفواصل شامبانيا ذهبية هادئة، بينما الواجهة الداخلية الداكنة تستخدم كحلياً عميقاً وأزرق كهربائياً مطابقين لاتجاه الواجهة الخارجية. الأزرق الواضح يبقى للإجراءات الوظيفية الأساسية.
- شعار N9 يتبدل آلياً في رأس الترحيب ولوحة الدخول وشريط التنقل الداخلي بحسب الثيم. قواعد الثيم الداخلي محصورة في Chrome التطبيق ولا تستهدف شاشة الهاتف أو دليل الرسالة أو `.export-capture`.
- واجهة الترحيب تعرض مسار العمل بثلاث خطوات مرتبة، وتُبقي تسجيل الدخول هو الإجراء الأساسي الواضح في الصفحة.
- الأصل البصري الخارجي هو `public/og.png`؛ يظهر كاملاً من دون قص النص أو إعادة رسم المجلدات والدروع بعناصر CSS.
- النسخة الفاتحة أصل بصري مستقل وليست فلتر CSS، وتُعرض بنفس أبعاد النسخة الداكنة. الخلفيتان ثابتتان للعرض فقط؛ لا تضف نقاط تفاعل فوق المجلدات أو أزرار ملفات منفصلة على الجوال من دون طلب جديد صريح.
...
### تغطية `composer-permissions.test.mjs`

- إخفاء منشئ الرسائل عن المستخدم العادي وحراسة معالج الواجهة.
- رفض إضافة أو تعديل أو حذف رسالة `sourceKind: "manual"` من مستخدم غير مشرف في Local وSites وVercel.
- السماح ببقاء رسائل المشرف اليدوية عند استيراد XML عادي من مستخدم مصرح.

### تغطية `conversation-export-utils.test.mjs`

- استخراج الروابط الفريدة من نص SMS العربي.
- بناء صف مرجع كامل للمحادثة من دون تغيير timestamps الأصلية.
- وضع عدة روابط في حقل مستقل، وتمييز الرسائل اليدوية والصادرة.
...
### تغطية `iphone-export.test.mjs`

- تثبيت تصدير iPhone كلقطة محادثة مختصرة، وليس شاشة تفاصيل Android/Huawei.
- منع رجوع العنوان وصفوف الحالة والنوع والأولوية وإطار الجهاز إلى لقطة iPhone.
...
### تغطية `theme-brand.test.mjs`

- التحقق من ملفات PNG الحقيقية للشعار اللؤلؤي والليلي والفافيكون وربطها بالواجهة و`index.html`.
- منع محددات الثيم الداخلي من استهداف شاشة الهاتف أو `.export-capture` أو مكونات الدليل.

### تغطية `export-experience.test.mjs`

- تثبيت مؤشرات Android/Huawei الواقعية: أشرطة شبكة متدرجة، Wi-Fi نسبي، وبطارية ذات امتلاء واضح.
- منع عودة نافذة Modal الحاجبة إلى تصدير المحادثة الكاملة، والتحقق من الإشعار المصغر وإعدادات الالتقاط المثبتة.

### تغطية `zip-utils.test.mjs`

- إنشاء ZIP قياسي متوافق يحافظ على المسارات العربية والملفات المستقلة.
- رفض تعديل الأرشيف بعد إغلاقه النهائي.
...
- اختبر أن المستخدم العادي لا يرى أي مدخل للمنشئ وأن الخادم يرفض تغيير الرسائل اليدوية حتى لو تم تجاوز الواجهة.
...
- [ ] تصدير المحادثة كاملة ومشاهدة النسبة من 0 إلى 100%.
- [ ] التأكد من وجود PDF مستقل لكل رسالة وExcel مرجعي داخل ZIP.
- [ ] التأكد من أن روابط كل SMS موجودة في عمود `روابط موجودة في الرسالة` في Excel.
...
- [ ] الثيم الفاتح يبدأ لؤلؤياً والشعار الفاتح واضح بحجم صغير.
- [ ] الثيم الداكن يعرض شعار N9 الليلي، وتبويب المتصفح يعرض favicon بدلاً من الكرة الأرضية.
- [ ] تبديل ثيم الموقع لا يغير شكل الهاتف أو لقطة الدليل المصدرة.
...
```text
أنت تستلم مشروع N9 SMS Web قائم ويعمل. قبل أي تعديل اقرأ AGENTS.md كاملاً ثم اقرأ N9-SMS-WEB-HANDOFF.md كاملاً، واعتبرهما مرجعاً ملزماً. افحص git status وحافظ على تغييرات المستخدم. لا تعيد بناء المشروع من الصفر. لا تخلط بيانات الشركات، ولا تغيّر XML timestamps الأصلية، ولا تقص المحادثات أو نتائج المطابقة، وحافظ على هوية المرسل EJADA. منشئ الرسالة للمشرف فقط، والاختيار اليدوي هو الافتراضي. لا تغيّر شاشة الهاتف أو لقطة الدليل أو آلية التصدير أثناء تعديل Chrome الموقع أو ثيمه. شغّل الاختبارات والبناء بعد التعديل؛ خط الأساس الحالي 45 اختباراً ناجحاً. استخدم Neon وPrivate Vercel Blob في إنتاج Vercel ولا تسمح بالرجوع الصامت إلى IndexedDB. لا تجعل أي نشر عاماً ولا توسع الوصول من دون موافقة صريحة. نفذ طلبي الجديد فقط، ثم حدّث هذا المستند إذا أضفت قراراً أو ميزة دائمة.
```
...
- منشئ الرسالة متاح للمشرف فقط ويحفظ داخل الشركة مع وقت إرسال واستلام/تسليم.
...
- الهوية الحالية: لؤلؤ/عاج وذهب في الفاتح، وكحلي/أزرق كهربائي في الداكن، مع شعاري N9 وفافيكون حقيقي.
- Chrome الموقع والثيم لا يغيران الهاتف أو دليل الرسالة أو `.export-capture`.
- الموقع خاص.
- الاختبارات الحالية 47/47.
...
- ملفات نسخة العمل: 48 ملفاً خارج `node_modules`, `dist`, `.git` وقت التحقق.
...
## 46. صلاحيات المحادثات لكل مستخدم — 10 سبتمبر 2026

يمكن للمشرف تقنين ما يراه المستخدم العادي داخل كل شركة، وليس مجرد تفويض الشركة كاملة.

### الاستخدام

1. افتح **المستخدمون** واختر بطاقة المستخدم العادي.
2. فوّض الشركة واحفظ صلاحيات الشركات أولًا.
3. افتح **تحديد المحادثات المسموح بها** واختر الشركة.
4. اختر **محادثات محددة فقط**، ثم ابحث باسم المحادثة أو رقم المرسل وحدد المحادثات المطلوبة.
5. اضغط **حفظ اختيار المحادثات**. يوجد تحديد لكل نتائج البحث ومسح للتحديد؛ القائمة تعرض دفعات دون إسقاط نتائج.

اختيار صفر محادثات يمنع ظهور كل رسائل هذه الشركة. خيار **كل المحادثات الحالية والمستقبلية** تفويض شامل صريح. المشرف يرى الكل دائمًا ولا يخضع لهذه القائمة.

### قواعد البيانات والأمان

- جدول إضافي `conversation_permissions` مفتاحه `(user_id, workspace_id)` ويحفظ `addresses` كنص JSON و`revision` للتزامن؛ موجود في Neon وD1، مع migrations رقم `0002`.
- JSON `null` يعني الكل، ومصفوفة تعني قائمة مرسلين مسموحة. السياسة التالفة تمنع الوصول. عدم وجود صف يحافظ على التفويض القديم الشامل عند ترقية المشروع.
- الحسابات العادية الجديدة وتفويضات الشركات المضافة حديثًا تبدأ بمصفوفة فارغة. حسابات ما قبل التحديث لا تُقيّد آليًا؛ يحدد المشرف نطاقها من الواجهة.
- هوية المحادثة هي عنوان المرسل بعد `normalizeMessageIdentity`، بنفس دمج EJADH/AMANA إلى EJADA في التطبيق. تسمح القائمة بكل الرسائل الحالية والمستقبلية لنفس المرسل، لا بمرسل جديد أو رسائل فردية مختارة.
- مسار الإدارة `GET/PATCH /api/users/:userId/workspaces/:workspaceId/conversations` خاص بالمشرف ويتحقق من عضوية الشركة وصحة المحادثات ونسخة الصلاحيات. التعديل المتزامن القديم يعاد بـ409.
- المستخدم العادي في Vercel لا يحصل على رابط Blob للأرشيف الأصلي، حتى مع تفويض الكل؛ يحصل على صفحات JSON مصفّاة من الخادم (1000 رسالة كحد أقصى للصفحة وبحد يقارب 3 MiB). تحقَّق من revision بين الصفحات ولا تعرض تحميلًا جزئيًا إذا تغير الأرشيف أو الصلاحيات.
- مستخدم النطاق المحدد لا يرى اسم ملف المصدر الأصلي أو عدد الرسائل المخفية. البحث والمطابقة والنسخ والتصدير تعمل على البيانات المصرح بها فقط؛ لا تتغير بيانات الرسائل أو تواريخها أو شكل تصدير الهاتف.
- الأرشيف المحدد للقراءة والبحث والتصدير فقط. رفع XML وكتابة الأرشيف ممنوعان في الواجهة والخادم؛ هذا يمنع استبدال الأرشيف الكامل بنسخة جزئية. Excel للمطابقة يبقى متاحًا.
- الواجهة تتحقق من صلاحيات المستخدم العادي كل 30 ثانية وعند العودة للنافذة، وتمسح العرض السابق أثناء تحميل النسخة الجديدة. البيانات التي سبق للمستخدم تنزيلها أو تصديرها لا يمكن سحبها من جهازه؛ هذا ليس نظام حذف عن بُعد.
- روابط Blob الصادرة من نسخة التطبيق السابقة قد تظل صالحة حتى انتهاء مدتها القصيرة؛ النسخة الجديدة لا تصدرها للمستخدمين العاديين.
- وضع التطوير المحلي يحاكي القواعد في IndexedDB لكنه ليس حدًا أمنيًا ضد مالك المتصفح؛ الضمان الفعلي في نشر الخادم.

### الملفات والتحقق

- المنطق المشترك: `src/conversationAccess.js`؛ الواجهة: `src/ConversationPermissionsEditor.jsx`.
- التخزين والمسارات: `server/vercelApi.js` و`worker/index.js` و`src/localWorkspaceStore.js`؛ العميل: `src/workspaceApi.js` و`src/App.jsx`.
- تجهيز Sites ينسخ المنطق المشترك و`dataUtils.js` إلى `dist/src` حتى تعمل استيرادات `dist/server/index.js`.
- اختبارات `tests/conversation-access.test.mjs` تغطي التصفية الدقيقة، التطبيع، البيانات الأصلية، المنع الافتراضي، الصفحات، منع كشف Blob، منع الكتابة، ومنع المستخدم العادي من إدارة الصلاحيات.
- لم تُعدَّل صلاحيات أي حساب فعلي أثناء التنفيذ. لا تفترض أن نجاح الاختبارات المحلية يعني نشر التحديث تلقائيًا على Vercel.
- تحقق التنفيذ: `npm test` ناجح 63/63، و`npm run build` و`npm run build:vercel` ناجحان. تشمل نسخة العمل أيضًا اختبارات استيراد XML غير المرتبطة بهذه الميزة. اختُبرت الواجهة على منفذ محلي منفصل بحساب تجريبي: تفويض EJADA وحدها أظهر محادثة واحدة وأربع رسائل، وأخفى بقية المحادثات ومنع رفع XML مع إبقاء Excel للمطابقة.

## 47. حزمة تحديث السحابة — 10 سبتمبر 2026

طلب المستخدم نشر التعديلات الحالية إلى GitHub/Vercel، بما فيها صلاحيات المحادثات وتعديلات استيراد XML الموجودة في نسخة العمل. لا تشمل الحزمة ملفات العملاء أو بيانات IndexedDB أو متغيرات البيئة السرية.

التدفق الأحدث للاستيراد يحل محل وصف DOMParser التاريخي في القسم 13: `src/xmlImport.worker.js` يقرأ الملف تدريجيًا في Web Worker باستخدام `saxes` و`src/xmlImport.js`. يدعم صيغة SMS Backup & Restore وصيغة `iphone_messages/messages/message`، ويحفظ النصوص والتواريخ الأصلية وGUID وبيانات iMessage المتاحة. يتطلب تاريخ iPhone ISO منطقة زمنية صريحة، أو يحول نانوثواني Apple منذ 2001 دون تقريب المصدر. يرفض XML غير المكتمل واختلاف عدد رسائل iPhone، ويمنع تكرار GUID عند دمج أجزاء الأرشيف. يبقى نطاق صلاحيات المستخدم هو الحاكم للاستيراد.

## 48. إصلاح بطء تحميل المستخدمين — 22 سبتمبر 2026

أكد اختبار حساب مستخدم مفوض أن الدخول ناجح (200) وأن 34,783 رسالة وصلت كاملة، لكن التحميل السابق استغرق نحو 88 ثانية عبر 35 صفحة مع إعادة قراءة Blob كاملًا في كل طلب. لا تعالج هذه المشكلة بتوسيع صلاحيات الحساب أو إصدار رابط الأرشيف الكامل له.

- `server/archiveCache.js` يحتفظ بقراءة كائن أرشيف واحد بمفتاحه غير القابل للاستبدال لمدة دقيقة، ويجمع القراءات المتزامنة. تغيير الشركة/الإصدار يغير المفتاح، ولا تُخزن صلاحيات المستخدم أو الاستجابات المصفاة في هذا المخزن. الفشل لا يتحول إلى أرشيف فارغ ولا يبقى مخزنًا.
- صفحات رسائل Vercel تستوعب حتى 10,000 رسالة مع بقاء حد 3 MiB الفعلي؛ كتالوج إدارة المحادثات يبقى 1000 عنصر. قيمة `total` تحصي الرسائل المسموحة فقط. العضوية والصلاحيات تُفحص قبل وبعد القراءة حتى مع cache hit.
- `src/archiveDownload.js` يجمع الصفحات، يبلغ عن عدد المحمل والإجمالي، ويرفض اختلاف الإصدار أو العدد أو تعطل المؤشر. لا ينشر أرشيفًا جزئيًا كأنه كامل.
- الواجهة تعرض تقدم التحميل وخطأ دائمًا مع إعادة المحاولة، وتلغي الطلبات السابقة عند تبديل الشركة/الخروج. حالة غياب تفويض المحادثات تطلب مراجعة المشرف بدل مطالبة المستخدم برفع XML محظور عليه.
- لا تغيير لكلمات المرور أو عضويات المستخدمين أو نطاق المحادثات أو بيانات الرسائل ضمن هذا الإصلاح.
