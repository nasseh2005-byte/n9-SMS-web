...
- The exterior landing art direction is “Secure Digital Archive”: deep navy surfaces, restrained electric blue and teal accents, glass-like institutional panels, and the supplied `public/og.png` artwork as the visual source of truth. Dark is the welcome default; light remains an optional saved preference. Use the real raster asset, not CSS or div approximations.
- The signed-out landing is a full-screen interactive image stage, not a permanent split hero/login layout. `public/og.png` is the dark source, `public/n9-hero-light.png` is the pearl-and-gold light companion, and `public/n9-logo.jpg` is the product mark. Archive trays have real hover/click hotspots, while sign-in is gated behind the lock-to-unlock transition and a slow side/bottom slide panel.
